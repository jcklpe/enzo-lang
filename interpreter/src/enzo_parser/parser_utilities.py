...
def get_code_line(src_lines, token, src):
    if hasattr(token, 'line') and token.line is not None:
        ...
        if 1 <= line_num <= len(src_lines):
            return src_lines[line_num - 1].rstrip()  # Only strip trailing whitespace, keep leading indentation and comments
    ...
    if hasattr(token, 'start') and token.start is not None and src:
        ...
        for line_num, line in enumerate(src_lines):
            ...
            if current_pos <= token.start < line_end:
                return line.rstrip()  # Only strip trailing whitespace, keep leading indentation and comments
            current_pos = line_end

    return (src_lines[0].rstrip() if src_lines else src.rstrip()) if src_lines or src else ""
